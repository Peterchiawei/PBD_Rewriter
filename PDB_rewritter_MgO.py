...
while i < (len(pdb_new)+1):
    ...
    if (i+2) > len(pdb_new)-1:   # Avoid exceptions when the last line is reached
        ...
        # Change the residue number of MgO
        desired_word_MG = 'MG'
        desired_word_O = 'O'
        if desired_word_MG in pdb_new[i-1][2] and desired_word_O in pdb_new[i][2]:
            for j in range(0, 9):
                if desired_word_MG in pdb_new[i-1-j][2]:
                    pdb_new[i+j][2] = 'O'
                    pdb_new[i+j][3] = 'MGO'
                    pdb_new[i+j][4] = molecular_counter
                    pdb_new[i-1-j][2] = 'MG'
                    pdb_new[i-1-j][3] = 'MGO'
                    pdb_new[i-1-j][4] = molecular_counter
                else:
                    break
            molecular_counter += 1
